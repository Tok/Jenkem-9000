...
object HtmlUtil {
  ...
  val META = "<meta http-equiv=\"content-type\" content=\"text/html; charset=" + CHARSET + "\">"

  def generateEmpty = DOCTYPE + "\n<html>\n<head>\n" + META + "\n<title></title>\n</head>\n" + "<body class=\"jenkemBody\"></body>\n</html>\n"

  def generateHtml(ircOutput: List[String], name: String, method: ConversionMethod) = {
    val html = new StringBuilder
    val css = new StringBuilder

    def generatePlain(line: String, lineId: Int) {
    ...
    def generate(line: String, lineId: Int) {
      html.append("<div class=\"jenkem\">")
      val sections = line.split(ColorUtil.CC)
      val filteredSections = sections.filterNot(_ == null).filterNot(_.equals("")).filterNot(_.equals(ColorUtil.BC))
    ...
    val lines = ircOutput.filterNot(_ == null).filterNot(_.equals(""))
    if (method.equals(ConversionMethod.Plain)) {
      (0 until lines.length).foreach(i => generatePlain(lines(i), i))
    } else { (0 until lines.length).foreach(i => generate(lines(i), i)) }
  ...
  def prepareHtmlForInline(inputHtml: String, inputCss: String) = {
    val filtered = inputHtml.split("\n").toList
  ...
  def prepareCssForInline(inputCss: String) = {
    val rep = ".jenkem { font-family: monospace; font-weight: bold; font-size: 10px; line-height: 12px; }"
    val filtered = inputCss.split("\n").toList.filterNot(_.startsWith("form {"))
      .filterNot(_.startsWith("body {")).filterNot(_.startsWith("html {"))
    val result = filtered.map(l => if (l.startsWith("div {")) { rep } else { l })
    "<style type=\"text/css\">\n" + result.map(l => l + "\n").mkString + "\n</style>"
  }

  def getHtmlUrl(name: String) = "/jenkem/output?name=" + name + ".html"
  def getCssUrl(name: String) = "/jenkem/cssOutput?name=" + name + ".css"
  def getIrcUrl(name: String) = "/jenkem/irc?name=" + name + ".txt"

  def escape(input: String) = input.replaceAll("&", "&amp;").replaceAll("<", "&lt;")
